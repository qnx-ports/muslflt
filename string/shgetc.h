...
void __musl_shlim(FILE *, off_t);
/* We only use a pseudo-FILE created from string, so __shgetc is not required */
/* int __shgetc(FILE *); */

#define shcnt(f) ((f)->shcnt + ((f)->rpos - (f)->buf))
#define shlim(f, lim) __musl_shlim((f), (lim))
#define shgetc(f) (((f)->rpos != (f)->shend) ? *(f)->rpos++ : EOF /* __shgetc(f) */)
